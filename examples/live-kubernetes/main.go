...
func main() {
	...
	podSecurityRule, err := celscanner.NewRuleBuilder("pod-security-context").
		...
		Build()
	if err != nil {
		log.Fatalf("Failed to build pod security rule: %v", err)
	}
	...
	resourceLimitsRule, err := celscanner.NewRuleBuilder("container-resource-limits").
		...
		Build()
	if err != nil {
		log.Fatalf("Failed to build resource limits rule: %v", err)
	}
	...
	privilegedRule, err := celscanner.NewRuleBuilder("privileged-containers").
		...
		Build()
	if err != nil {
		log.Fatalf("Failed to build privileged containers rule: %v", err)
	}
	...
	namespaceRule, err := celscanner.NewRuleBuilder("namespace-count").
		WithKubernetesInput("namespaces", "", "v1", "namespaces", "", "").
		...
		Build()
	if err != nil {
		log.Fatalf("Failed to build namespace rule: %v", err)
	}
